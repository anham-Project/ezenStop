...
<script type="text/javascript">
	...
	function deleteReply(r_num, a_num) {
		location.href = "trade_reply_delete.board?reply_num=" + r_num
				+ "&article_num=" + a_num;
	}
...
<div class="container" style="margin-top: 30px; margin-bottom: 10px;">
	<h5>
		<font size="3">이젠'sTop게시판 > <a href="trade_list.board" class="blackA">사고팔고 게시판</a> >
		</font><b>게시글보기</b>
	</h5>
	<div class="row"
		...
		<div class="col-md-3">
			<c:if test="${tradeDetail.grade == 1 }">
				<img class="media-object img-circle"
					...
					src="resources/img/${tradeDetail.academyLocation }.png">
			</c:if>
			<c:if test="${tradeDetail.grade != 1 }">
				<img class="media-object img-circle"
					style="width: 30px; height: 30px;"
					src="resources/img/${tradeDetail.grade }.png">
			</c:if>
			<c:choose>
		...
		<script>
			...
			function reReInput(num, id) {
				document.getElementById('inputReply').innerHTML = "<div class='input-group' style='padding:5px 5px 5px 5px; border:1; border-color:#F7F7F7;'>"
						...
						+ "<input class='btn btn-secondary btn-sm' type='submit' value='댓글달기' <c:if test='${empty sessionScope.userId}'>disabled</c:if>>"
						+ "</div>"
			}
			function deleteTag() {
		...
		$.ajax({
			...
			success: function(result){
				if(result === '-2'){
					...
					location.href="trade_list.board"
				}else{
