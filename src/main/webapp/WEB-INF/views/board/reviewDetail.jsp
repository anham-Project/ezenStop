...
<c:if test="${sessionScope.userId == reviewDetail.id}">
	<div class="row">
		...
		<button class="btn btn-info btn-sm" type="button" onclick="review_edit('${reviewDetail.article_num}')">글수정</button>
		<button class="btn btn-warning btn-sm" type="button" onclick="review_delete('${reviewDetail.article_num}')">글삭제</button>
		</div>
	</div>
</c:if>
<c:if test="${sessionScope.userId != reviewDetail.id}">
	<div class="row">
		...
		<button class="btn btn-info btn-sm" type="button" 
				onclick="inform('${reviewDetail.article_num}','${sessionScope.userId}','1')">추천</button>
		<button class="btn btn-secondary btn-sm" type="button"
				onclick="inform('${reviewDetail.article_num}','${sessionScope.userId}','2')">비추천</button>
		<button class="btn btn-warning btn-sm" type="button"
				onclick="inform('${reviewDetail.article_num}','${sessionScope.userId}','3')">신고하기</button>
		</div>
	</div>
</c:if>
	<hr/>
	...
	<div class="row" style="padding:5px 5px 5px 5px; background-color:#F7F7F7;">
		<div class="col-md-12" align="left">${dto.id} <font color="gray">${dto.regdate} ${dto.regdate_time} | </font>
			...
			<a href = "javascript:deleteReply('${dto.reply_num}','${reviewDetail.article_num}');">[삭제]</a>
			</c:if>
		</div>
		<div class="col-md-12" align="left">${dto.content}</div>
		<div class="col-md-12" align="right" id="buttontype">
		<input type="button" class="btn btn-info btn-sm" value="답글쓰기" onclick="addInput('${dto.reply_num}','${dto.id}');"/></div>
	</div>
	</c:forEach>
	</c:if>
<script>
