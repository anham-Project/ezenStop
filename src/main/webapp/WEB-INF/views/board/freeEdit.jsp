...
<div class="container" style="margin-top:30px; margin-bottom:10px;">
	...
	<h5><font size="3">이젠'sTop게시판 > <a href="free_list.board" class="blackA">자유게시판</a> > </font><b>게시글 수정</b></h5>
	<div class="container" style="margin-top:30px; margin-bottom:10px;">
	<form class="form-horizontal" name ="f" action="free_edit.board" method="post" 
		onsubmit="return check()" enctype="multipart/form-data">
		<input type="hidden" name="article_num" value="${freeDetail.article_num}">
	<div class="input-group" style="padding-bottom:5px">
		<label class="col-md-2 control-label">카테고리<font size="3" color="red">*</font></label>
		<div class="col-md-3">
			<select class="form-control" name="category">
			<c:forEach var="list" items="${categoryList}">
			<option <c:if test="${freeDetail.category == list}">selected</c:if>>${list}</option>
			</c:forEach>
			</select>
	...
	<div class="input-group" style="padding-bottom:5px">
		<input class="form-control" type="text" name="subject" value="${freeDetail.subject}">
	</div>
	<div class="input-group" style="padding-bottom:5px">
		<textarea class="form-control" rows="13" name="content">${freeDetail.content}</textarea>
	</div>
    <div class="input-group" style="padding-bottom:5px">
    	<font color="gray">첨부된 파일 :</font>
    	<div id="delbefore">
    	<c:if test="${freeDetail.filesize != 0}"><b>&nbsp;${freeDetail.image}&nbsp;&nbsp;</b>
    		<input type="hidden" name="image0" value="${freeDetail.image}">
    		<input type="hidden" name="filesize0" value="${freeDetail.filesize}">
    		<button class="btn btn-outline-secondary btn-sm" type="button" onclick="javascript:deleteImage()">파일삭제</button>
    	</c:if>
    	</div>
    	<c:if test="${freeDetail.filesize == 0}"><b>&nbsp;첨부파일이 없습니다.&nbsp;&nbsp;</b>
